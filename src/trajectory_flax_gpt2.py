...
class FlaxGPT2PreTrainedModel(FlaxPreTrainedModel):
    ...
    def init_weights(self, rng: jax.random.PRNGKey, input_shape: Tuple, params: FrozenDict = None) -> FrozenDict:
        ...
        ### MODIFIED: No need for input embedders since we will directly feed embeddings as inputs
        input_embeds = jnp.zeros(input_shape + (self.config.n_embd,))
        ...
        if self.config.add_cross_attention:
            ...
            module_init_outputs = self.module.init(rngs, input_embeds, attention_mask, return_dict=False)
    ...
    def init_cache(self, batch_size, max_length):
        ...
        ### MODIFIED: No need for input embedders since we will directly feed embeddings as inputs
        input_embeds = jnp.zeros((batch_size, max_length, self.config.n_embd))
        attention_mask = jnp.ones((batch_size, max_length))
        # input_ids = jnp.ones((batch_size, max_length))
        # attention_mask = jnp.ones_like(input_ids)
        # position_ids = jnp.broadcast_to(jnp.arange(jnp.atleast_2d(input_ids).shape[-1]), input_ids.shape)

        init_variables = self.module.init(
            jax.random.PRNGKey(0), input_embeds, attention_mask, return_dict=False, init_cache=True
        )
