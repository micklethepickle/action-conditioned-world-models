...
def make_loss(policy_network: Any,
	...
	def step(carry: Tuple[Any, PRNGKey], tmp, policy):
		...
		action_obs = detach(obs)
		action, extra = policy(action_obs, key_sample) # action of shape ( -1)
	...
	def batch_policy_loss(policy_params, preprocess_params, init_obs, entropy_reg, key):
		...
		return -(averaged_returns + (entropy_reg*total_entropy*(1-deterministic_policy))), {'entropy':total_entropy, 'img_ret': averaged_returns}
