from src.brax.arm_icml_rebuttal_variedlengths import networks as arm_networks
...
def make_losses(arm_networks: arm_networks.ARMNetworks, 
	...
	def dynamics_loss(dynamics_params, preprocess_params, observations, actions, rewards, next_observations, key):
		...
		if difference_transition:
			# exclude timestep prediction
			next_obs_predictions = jnp.concatenate((next_obs_predictions[:, :, :-1] + observations[:, :, :-1],
										  next_obs_predictions[:, :, -1:]), axis=-1)
		...
		return total_loss, {'tloss': tloss, 'rloss': rloss}
	...
	def step(carry: Tuple[Any, PRNGKey], tmp, policy, preprocess_params, transition_params, reward_params, other_policy=None):
		...
		action_obs = detach(obs) + (bp_discount * obs) - (bp_discount * detach(obs))
		# bp_discounted_obs = detach(obs) + (bp_discount * obs) - (bp_discount * detach(obs))
		# bp_discounted_obs = obs
		action, extra = policy(action_obs, key_sample) # action of shape (batch_size, 1, -1)
		...
		if difference_transition:
			next_obs = jnp.concatenate((next_obs[:, :, :-1] + obs[:, :, :-1],
										  next_obs[:, :, -1:]), axis=-1)
	...
	def batched_get_imagined_out(policy_params, preprocess_params, transition_params, reward_params, critic_params, 
		...
		if other_policy_params is not None:
			other_policy = make_policy((preprocess_params, other_policy_params), get_dist=True, deterministic=deterministic_policy)
		else:
			other_policy = None
		f = functools.partial(step, 
							policy=make_policy((preprocess_params, policy_params), get_dist=True, deterministic=deterministic_policy),
		...
		if unroll_length > 1:
			...
			rewards = jnp.transpose(rewards, axes=(1, 0))
		...
		if unroll_length > 1:
			...
			total_entropy = jnp.squeeze(entropy)
			total_divergence = jnp.squeeze(divergence) # end shape (batch_size)
	...
	def batch_policy_loss(policy_params, preprocess_params, transition_params, 
		...
		return -(averaged_returns + (entropy_reg*total_entropy*(1-deterministic_policy))), {'target_value': target_value, 'entropy':total_entropy, 'img_ret': averaged_returns,
